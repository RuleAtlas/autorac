...
<akomaNtoso xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0">
  <doc>
    ...
    def test_run_akn_section_eval_passes_source_metadata_sidecar(
...
cases:
  ...
  - kind: source
    ...
    def test_repo_us_snap_tx_standard_utility_allowance_refresh_manifest_loads_expected_case(
        ...
        assert (
            case.metadata_file
            == tx_target_root / "snap_standard_utility_allowance_tx.meta.yaml"
        )
    ...
    def test_repo_us_snap_tx_limited_utility_allowance_refresh_manifest_loads_expected_case(
        ...
        assert (
            case.metadata_file
            == tx_target_root / "snap_limited_utility_allowance_tx.meta.yaml"
        )
    ...
    def test_repo_us_snap_tx_individual_utility_allowance_refresh_manifest_loads_expected_case(
        ...
        assert (
            case.metadata_file
            == tx_target_root / "snap_individual_utility_allowance_tx.meta.yaml"
        )
    ...
    def test_repo_us_snap_tx_child_support_deduction_option_refresh_manifest_loads_expected_case(
        ...
        manifest = load_eval_suite_manifest(
            repo_root
            / "benchmarks"
            / "us_snap_tx_child_support_deduction_option_refresh.yaml"
        )

        assert manifest.name == "Texas SNAP child support deduction option refresh"
        assert manifest.mode == "repo-augmented"
        assert len(manifest.cases) == 1
        assert manifest.gates.min_policyengine_pass_rate == 1.0
        tx_target_root = (
        ...
        assert (
            case.metadata_file
            == tx_target_root / "snap_state_uses_child_support_deduction_tx.meta.yaml"
        )
    ...
    def test_repo_us_snap_tx_self_employment_expense_option_refresh_manifest_loads_expected_case(
        ...
        manifest = load_eval_suite_manifest(
            repo_root
            / "benchmarks"
            / "us_snap_tx_self_employment_expense_option_refresh.yaml"
        )

        assert manifest.name == "Texas SNAP self-employment expense option refresh"
        assert manifest.mode == "repo-augmented"
        assert len(manifest.cases) == 1
        assert manifest.gates.min_policyengine_pass_rate == 1.0
        tx_target_root = (
        ...
        assert (
            case.metadata_file
            == tx_target_root
            / "snap_self_employment_expense_based_deduction_applies_tx.meta.yaml"
        )
        assert case.section_eid is None
    ...
    def test_repo_us_snap_tx_standard_medical_expense_deduction_refresh_manifest_loads_expected_case(
        ...
        assert (
            case.metadata_file
            == tx_target_root / "snap_standard_medical_expense_deduction_tx.meta.yaml"
        )
        assert (
            case.section_eid
    ...
    def test_repo_us_snap_tx_tanf_non_cash_asset_limit_refresh_manifest_loads_expected_case(
        ...
        assert (
            case.metadata_file
            == tx_target_root / "snap_tanf_non_cash_asset_limit_tx.meta.yaml"
        )
...
class TestRepoAugmentedContext:
    ...
    def test_load_source_text_for_eval_prefers_akn_backing(self, tmp_path, monkeypatch):
        ...
        akn_file = arch_root / "us-tx" / "tx" / "akn" / "source.akn.xml"
        akn_file.parent.mkdir(parents=True, exist_ok=True)
        akn_file.write_text(
            """
<akomaNtoso xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0">
  <doc name="doc">
    <mainBody>
      <hcontainer name="section" eId="sec_sua">
        ...
        source_path.with_name("snap_sua.meta.yaml").write_text(
            ...
            "source_backing:\n"
            "  kind: akn_section\n"
            "  arch_path: us-tx/tx/akn/source.akn.xml\n"
            "  section_eid: sec_sua\n"
            "relations:\n"
            "  - relation: sets\n"
            "    target: cfr/7/273.9/d/6/iii#snap_standard_utility_allowance\n"
    ...
    def test_load_source_text_for_eval_supports_multiple_akn_sections(
        self, tmp_path, monkeypatch
    ):
        arch_root = tmp_path / "arch"
        monkeypatch.setenv("AUTORAC_ARCH_ROOT", str(arch_root))
...
<akomaNtoso xmlns="http://docs.oasis-open.org/legaldocml/ns/akn/3.0">
  <doc name="doc">
    ...
    def test_load_source_text_for_eval_includes_block_list_items(
        self, tmp_path, monkeypatch
    ):
        arch_root = tmp_path / "arch"
        monkeypatch.setenv("AUTORAC_ARCH_ROOT", str(arch_root))
...
class TestSourceEval:
    ...
    def test_build_eval_prompt_includes_sets_source_metadata_guidance(self, tmp_path):
        ...
        source_path.with_name("snap_standard_utility_allowance_tn.meta.yaml").write_text(
            ...
            "source_backing:\n"
            "  kind: akn_section\n"
            "  arch_path: us-tn/tn/akn/source.akn.xml\n"
            "  section_eid: sec_sua\n"
            "relations:\n"
            "  - relation: sets\n"
            "    target: cfr/7/273.9/d/6/iii#snap_standard_utility_allowance\n"
